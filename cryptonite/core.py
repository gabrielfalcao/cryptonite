...
from cryptonite.serializers import GPGSerializer
...
class SymmetricalHelper(object):
    ...
    def encrypt(self, data):
        ...
        logger.warning(
            'the "fernet_key" setting is not available. The data will not be encrypted')
        return data
...
def ensure_secure_key_folder(path):
    ...
    if mode != '700':
        ...
        logging.warning('changing mode of {0} to 0700'.format(
...
class Cryptonite(object):
    ...
    def __init__(self, path=conf.key_home, password=conf.fernet_key):
        # this has a side-effect in the file-system
        self.key_home = ensure_secure_key_folder(path)
    ...
    def sort_keys(self, keys):
        ...
        """
        ordered = sorted(sorted(keys, key=lambda x: x.get(
            'email', '')), key=lambda x: x.get('private', ''), reverse=True)
        uniq = []
        final = []
        for key in ordered:
            keyid = key['keyid']
    ...
    def encrypt(self, recipient, plaintext, sign_from=None):
        ...
        if not fingerprint:
            msg = 'there are no keys for the recipient for email: {0}'.format(
                recipient)
        ...
        crypt = self.gpg.encrypt(
            plaintext.strip(), recipient, always_trust=True, **kw)
        if crypt.data:
            return crypt.data
    ...
    def decrypt(self, ciphertext, passphrase=None):
        ...
        crypt = self.gpg.decrypt(
            ciphertext.strip(), passphrase=passphrase, always_trust=True)
        if crypt.data:
            return crypt.data
        ...
        if not key['private']:
            raise InvalidKeyError(
                'cannot decrypt because private key is missing for: {0}'.format(uid['email']))
